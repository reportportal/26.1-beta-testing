---
name: reportportal-portfolio-dashboard
description: >-
  Builds a portfolio / go-no-go dashboard across ReportPortal projects
  using MCP (Manual, Automated, Agentic). Use when the user asks for release
  readiness, org roll-up, go/no-go, test plan coverage, or a
  ReportPortal portfolio dashboard.
---
...
2. **Pull data** (read-only):
   - `get_launches` — automated + agentic (use `launchType` / CheckSuite attributes)
...
3. **Classify launches**
   - **Automated**: `launchType=AUTOMATION` without CheckSuite / `test_session` markers
...
4. **Compute valid metrics** — follow [reference.md](reference.md). Do **not** blend Manual + Automated + Agentic into one org pass rate.
...
|------|-------------|-----------|------------|
| Automated | Latest suite per project: status, freshness (≤7d default), open PB/TI | Pass % of that latest run; AB/SI separately | Lifetime pass %; blend with Manual |
...
Write to the workspace canvases dir as `portfolio-dashboard.canvas.tsx`.
...
5. **Health by type** — UsageBars for Automated / Manual / Agentic  
6. **Automated tests charts** — stacked executions + defect taxonomy; small detail table  
...
- Repo copy-paste prompt: `prompts/portfolio-dashboard.md` (repository root)
